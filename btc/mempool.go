...
func mempoolTransaction(inTx *btcjson.TxRawResult) {
	...
	for _, input := range inTx.Vin {
		...
		for _, output := range previousTxVerbose.Vout {
			for _, address := range output.ScriptPubKey.Addresses {
				query := bson.M{"wallets.addresses.address": address}
				err := usersData.Find(query).One(&user)
				if err != nil {
				...
				case mgo.ErrNotFound:
					// add new
					newRec := newTxRecord(user.UserID, inTx.Txid, inTx.Hash, output.ScriptPubKey.Hex, address, TxStatusAppearedInMempoolOutcoming, int(output.N), -1, output.Value)
					err = txsData.Insert(newRec)
					if err != nil {
						log.Errorf("parseNewBlock:outputsData.Insert mgo.ErrNotFound: %s", err.Error())
					}
				case nil:
					// append
					sel := bson.M{"userid": user.UserID, "outputs.txid": inTx.Txid, "outputs.txaddress": address}
					err = txsData.Find(sel).One(nil)
					switch err {
					case mgo.ErrNotFound:
						newTx := newMultyTX(inTx.Txid, inTx.Hash, output.ScriptPubKey.Hex, address, TxStatusAppearedInMempoolOutcoming, int(output.N), -1, output.Value)
						sel = bson.M{"userid": user.UserID}
						update := bson.M{"$push": bson.M{"transactions": newTx}}
						err = txsData.Update(sel, update)
						if err != nil {
