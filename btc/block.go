					...
					case nil:
						...
						update := bson.M{
							"$set": bson.M{
								...
								"outputs.$.txblockheight": blockHeight,
							},
						}
						err = txsData.Update(sel, update)
						if err != nil {
		...
		for _, input := range blockTxVerbose.Vin {
			...
			for _, output := range previousTxVerbose.Vout {
				for _, address := range output.ScriptPubKey.Addresses {
					...
					case mgo.ErrNotFound:
						// add new
						newRec := newTxRecord(user.UserID, blockTxVerbose.Txid, blockTxVerbose.Hash, output.ScriptPubKey.Hex, address, TxStatusAppearedInBlockOutcoming, int(output.N), blockHeight, output.Value)
						err = txsData.Insert(newRec)
						if err != nil {
							log.Errorf("parseNewBlock:outputsData.Insert mgo.ErrNotFound: %s", err.Error())
						}
					case nil:
						...
						case mgo.ErrNotFound:
							...
							newTx := newMultyTX(blockTxVerbose.Txid, blockTxVerbose.Hash, output.ScriptPubKey.Hex, address, TxStatusAppearedInBlockOutcoming, int(output.N), blockHeight, output.Value)
